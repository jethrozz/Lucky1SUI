...
export const getUsetTickets = async (address: string, graphqlUrl: string): Promise<Array<LotteryTicket>> => {
    ...
    // 在客户端对JSON字段进行过滤
    const filteredNfts = nfts?.map(jsonString => getLotteryTicket(jsonString)).filter(ticket => ticket != null);
    return filteredNfts || [];
}
