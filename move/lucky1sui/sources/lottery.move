module lucky1sui::lottery {
    ...
    */
    ...
    fun init(ctx: &mut TxContext) {
        ...
        transfer::share_object(ticket_pool);
    ...
    public entry fun startFirstLottery(_: &LotteryAdminCap, lottery: &mut Lottery, clock : & Clock, ctx: &mut TxContext){
        ...
        let lottery_pool = createLotteryPool(ctx, clock);
        let user_count = lottery_pool.user_deposit.size();
    ...
    public entry fun joinLotteryPool<CoinType>(
        ...
        while(i < vector::length(ticket_number_set)){
            ...
            lottery_pool.joined_ticket_numbers.insert(*ticket_no, ticket_id);
            i=i+1;
        };
        // 获取 account_cap 的引用
        //拿到资产索引
        ...
        if(lottery_pool.user_deposit.contains(&ctx.sender())){        
            let old_value = lottery_pool.user_deposit.get(&ctx.sender());
            let total = *old_value + deposit_coin_value;

            lottery_pool.user_deposit.remove(&ctx.sender());
            lottery_pool.user_deposit.insert(ctx.sender(), total);
        }else{
            lottery_pool.user_deposit.insert(ctx.sender(), deposit_coin_value);
        };
